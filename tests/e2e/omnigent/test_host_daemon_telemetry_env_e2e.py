...
``CLAUDE_CODE_ENABLE_TELEMETRY=1`` (plus the
``OTEL_*`` exporter config and ``OMNIGENT_RUNNER_ENV_PASSTHROUGH`` naming the
variable) and then starts Omnigent in the background finds the flag missing
...
# A generic extra name forwarded via OMNIGENT_RUNNER_ENV_PASSTHROUGH, proving
# the passthrough lane works for arbitrary operator-named variables too.
_CONTROL_VAR = "OMNIGENT_TEST_PASSTHROUGH_CONTROL"
...
def _telemetry_env(base_env: dict[str, str], home: Path) -> dict[str, str]:
    ...
    env[_CONTROL_VAR] = "control-value"
    env["OMNIGENT_RUNNER_ENV_PASSTHROUGH"] = f"CLAUDE_CODE_ENABLE_TELEMETRY,{_CONTROL_VAR}"
    return env
